 ...
 *   Fonte de verdade: pesquisas_clima.json + respostas_clima.json.
...
var EscutaRepository = (function() {
  ...
  function salvarPesquisa(orgId, dados) {
    ...
    modifyJSON(ARQUIVO_PESQUISAS, function(lista) {
      ...
      var registro = Object.assign({
        id: id, orgId: orgId, criadoEm: agora
  ...
  function metricasPesquisas(orgId) {
    ...
    var concluidas = lista.filter(function(p) { return p.status === 'concluida'; }).length;
  ...
  function sincronizarSheet(orgId, pesquisaId, resumo) {
    ...
    if (!aba) return;
    // remove linha anterior deste pesquisaId

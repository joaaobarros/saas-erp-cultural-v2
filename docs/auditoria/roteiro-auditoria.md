...
| ~~47~~ | ~~ESP-04~~ | Infraestrutura — Nova Reserva | ~~Botão "Lote" posicionado na barra de ações~~ **CORRIGIDO F69 @516**: botão movido para o cabeçalho do campo "Data *" (label row flex); botão "Checar disponibilidade" removido — substituído por checagem automática em tempo real. | ~~🟡 Média~~ |
...
| ~~57~~ | ~~ESP-14~~ | Infraestrutura — Inconsistência | ~~Campo "Tipo" no modal Lote é select; formulário principal é texto livre~~ **CORRIGIDO F69 @516**: `res-tipo-acao` migrado de `<input type="text">` para `<select>` com 13 opções padronizadas — consistente com o tipo do modal Lote; modal Lote agora herda o valor do formulário principal. | ~~🔴 Alta~~ |
...
| ~~203~~ | ~~ADM-11~~ | Admin — Provisionamento | ~~Botão "Abrir Wizard de Setup" abre página em branco~~ **CORRIGIDO s17 F63 @490**: `irPara()` em `wizard_setup.html` caps em 7 — `_calcularPassoAtual` retornava 9 (checks.length+1) para org 100% provisionada, mas só existem passos 1–7. | ~~🟡 Média~~ |
...
| ~~155~~ | ~~ADM-04~~ | Admin — Banco de Dados | ~~Aba "Banco de Dados" visível para Admin mas inacessível~~ **CORRIGIDO s17 F63 @490**: botão da tab oculto para não-SuperAdmin em `aoAbrir()` + guard em `abrirTab('dados')` bloqueia carga com mensagem explicativa. | ~~🟡 Média~~ |

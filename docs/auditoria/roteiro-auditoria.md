...
| 261 | CAR-15 | Reserva de Veículo — Papéis de Aprovação | **Aprovação de infra exige verificação de papel + setor — código atual só verifica papel.** Regra de negócio confirmada: podem aprovar reservas de veículo e demais solicitações de infraestrutura: (1) papel `infraestrutura` — sempre; (2) papel `gestor` vinculado ao setor Infraestrutura; (3) papel `admin` vinculado ao setor Infraestrutura; (4) `superadmin` — sempre (aprovação universal). O código atual usa `PAPEIS_APROVACAO = ['infraestrutura', 'gestor', 'admin', 'superadmin']` sem checar setor — um gestor de Ação Cultural pode aprovar uma reserva de veículo, violando a regra. Correção: `_podAprovar(email)` deve verificar `papel === 'infraestrutura' OR papel === 'superadmin' OR (papel IN ['gestor','admin'] AND setor === 'infraestrutura')` | 🟡 Média |

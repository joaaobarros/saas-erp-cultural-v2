...
| 74 | SIS-01 | **SISTEMA GLOBAL** | **Ocorrências de `prompt()` / `confirm()` nativos** — substituídos progressivamente: ✅ CHV-03 (s16 F21) · ✅ AlmoxUI._devolver (s16 F25) · ✅ SolicitacoesUI.devolver (s16 F25). Restam: Almox._cancelar, Contratações (pareceres/rejeição/instruir/execução/conclusão/cancelar/pagamento), Ativos manutenção, Aprovações recusa/papel, Agentes suspensão, e outros. | 🔴 Crítico |

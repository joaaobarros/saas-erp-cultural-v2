...
| 256 | SIS-14 | Sistema Global — Datas | **Datas ISO → pt-BR** — **PARCIALMENTE CORRIGIDO s16 Fase 11**: cards da lista e modal de detalhes de Reserva de Veículo agora usam `fmtDataPtBR()`. Auditoria sistêmica em demais módulos pendente. `fmtDataPtBR()` já existe globalmente e está disponível para uso. | 🟡 Média |

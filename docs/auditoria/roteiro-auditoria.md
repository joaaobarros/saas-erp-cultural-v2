...
| Aba Config. Sistema → Expediente & Horários | ✅ 08:00–21:30 + Salvar | 2026-05-31 s10 |
| Aba Config. Sistema → Identidade Visual | ✅ logo + extração automática de cores + derivadas automáticas | 2026-05-31 s10 |
...
| Aba Painel — carrega (loading infinito parcial) | ⚠️ ESC-04 | 2026-05-31 s11 |
...
| Aba Gestão — MARCADORES METODOLÓGICOS | ⚠️ ESC-05 "object Object" exibido | 2026-05-31 s11 |
...
| FSM do contrato — botões de transição na UI | ⚠️ FIN-14 — sem botões; transição via select no form editar | análise código s10 |
| Ícone de pessoas no card | ⚠️ FIN-15 CONFIRMADO — manage_accounts → abre painel detalhes; FIN-16 card Custo Mensal ausente; FIN-17 Benefícios incorretos (R$525,74 vs esperado R$1.191,33) | 2026-05-31 s10 |
| Modal "Editar Pessoal" — campos mapeados | ✅ confirmado via screenshot | 2026-05-31 s10 |
| FIN-01 (Setor não persiste) | ✅ CORRIGIDO commit 0e9317f — incluir em próximo teste visual | — |
...
| Motorista — quem dirige? | 🔲 pergunta pendente | — |
| Número de veículos na frota | 🔲 pergunta pendente | — |
...
- **Config → Horários**: horário global configurado 08:00–21:30 ✅ — mas validação usa 07:00 hardcoded (ESP-16 bug confirmado)
...
- **ACO-09**: 8 abas no detalhe da ação — Visão Geral, Tarefas, Reservas, Contratos, Equipe, Financeiro, Contratações, Mapa do Evento — sem scroll indicator visual; as abas 6–8 ficam ocultas sem sinalização de que existem mais abas à direita
...
**ADM-02** `🔴 Alta` *(REVISADO sessão 10)* — ~~Sem granularidade~~: modal "Editar usuário" **TEM** seção "Funcionalidades Específicas" com controles por funcionalidade dentro de cada módulo (5 grupos identificados: COMUNICAÇÃO, TAREFAS, PESSOAS, REUNIÕES, AÇÕES). Override ativo = roxo. **Gap residual confirmado:** (a) sem controle por setor/recurso (gestor X não consegue ver apenas pessoas do seu setor); (b) sem grupos de permissão — overrides são individuais por usuário; (c) cobertura de funcionalidades por módulo pode ser incompleta — quantos módulos têm funcionalidades específicas listadas ainda a confirmar. A granularidade existe; é mais completo do que registrado anteriormente.
...
**ADM-07** `🟡 Média` — **Barra de 10 abas sem sinalização de scroll** — a tab bar de "Cadastros Base" tem 10 abas (Espaços, Setores, Turnos, Categ.Itens, Módulos, Features, Provisionamento, Usuários, Config.Sistema, Banco de Dados) que não cabem na largura da tela sem scroll horizontal. Nenhum indicador visual (chevron, fade, seta, badge "+ N") sinaliza que existem abas fora da área visível — usuário pode não perceber as abas à direita.

**ADM-08** `🟡 Média` — **Turnos e Config.Sistema devem ser unificados** — Turnos define intervalos horários nomeados (Manhã/Tarde/Noite com horário e dias); Config.Sistema → Expediente & Horários define o horário global de funcionamento (08:00–21:30). Ambos configuram a dimensão temporal institucional e deveriam estar na mesma seção "Horários & Turnos". Confirmado pelo usuário.
...
- **Sem campo de motorista** — modelo de dados não registra quem conduz
...
### Perguntas pendentes
- **Motorista**: quem conduz o veículo? Existe motorista fixo de infraestrutura, ou o próprio solicitante dirige?
...
| 51 | ESP-08 | Infraestrutura — Nova Reserva | Campo "Setor" não é auto-preenchido — deveria carregar automaticamente o setor do usuário solicitante, com possibilidade de edição manual para casos excepcionais | 🟡 Média |
...
| 53 | ESP-10 | Infraestrutura — Select de Espaços | Select mostra **todos** os 17 espaços cadastrados, incluindo os não habilitados para reserva — deveria filtrar e exibir apenas espaços marcados como "disponível para reserva" na configuração | 🔴 Alta |
...
| 56 | ESP-13 | Infraestrutura — Lote | Modal "Criar Reservas em Lote" **duplica campos** do formulário principal (Nome do Evento, Tipo, Setor, Observações) — fluxo correto: preencher o formulário principal → clicar Lote → modal pergunta **somente** Espaço + Hora Início/Fim + padrão de datas. Demais info já está no formulário pai | 🔴 Alta |
...
| 58 | ESP-15 | Infraestrutura — Lote | **Política de conflito errada**: sistema cancela o lote inteiro se qualquer data tiver conflito — comportamento correto é: (1) verificar todas as datas; (2) exibir as datas em conflito; (3) perguntar "criar apenas as datas válidas?"; (4) criar somente as sem conflito se o usuário confirmar | 🔴 Alta |
| 59 | ESP-16 | Infraestrutura — Validação | **Bug confirmado**: horário configurado é 08:00–21:30 mas validação exibiu "anterior à abertura (07:00)" — sistema usa valor hardcoded (07:00) e ignora o horário global configurado em Config → Horários. Agenda e Diagrama também não refletem o intervalo configurado | 🔴 Alta |
| 59b | ESP-16b | Sistema — Configuração duplicada | Horário global de funcionamento existe em **dois lugares com duas interfaces de edição**: Infraestrutura → Config → Horários E Admin → Config Sistema → Expediente & Horários — ambos mostram 08:00–21:30. Risco de divergência caso um seja salvo e o outro não; deve existir em apenas um lugar (Admin) com Infraestrutura exibindo como leitura | 🔴 Alta |
...
| 74 | SIS-01 | **SISTEMA GLOBAL** | **56 ocorrências** de `prompt()` / `confirm()` nativos do browser em todo o sistema — aparecem como caixas do Google (URL exposta, sem identidade visual, sem estilo). Afetam: Chaves (devolução, cancelamento), Empréstimos (devolução), Patrimônio (manutenção), Financeiro (pareceres, rejeições, datas de pagamento), Aprovações (recusa/papel), Contratações (parecer gestor/financeiro, rejeição, devolução, conclusão, cancelamento), Agentes (suspensão), e outros. Todos devem ser substituídos por modais programados dentro do sistema, com campos tipados, validação e identidade visual consistente. | 🔴 Crítico |
| 64 | ESP-20 | Infraestrutura — Lista | Filtro de data **não dispara automaticamente** ao ser alterado — requer clicar no botão refresh para atualizar a lista; deveria filtrar em tempo real ao mudar data ou status | 🟡 Média |
| 65 | ESP-21 | Infraestrutura — Lista | Sem botão **"Redefinir filtros"** — usuário não consegue limpar todos os filtros de uma vez e voltar ao estado padrão | 🟡 Média |
| 66 | ESP-22 | Infraestrutura — Reservas | **Auto-conclusão ausente**: reservas em status "Em uso" não são automaticamente concluídas após o horário de fim — confirmado com reserva de 21/05/2026 ainda "Em uso" 10 dias depois. Comportamento esperado: concluir automaticamente 15 minutos após o horário de término caso ninguém confirme manualmente | 🔴 Alta |
...
| 69 | ESP-25 | Infraestrutura — Mapa | Campo de data no modo Mapa tem **fonte/estilo diferente** dos demais inputs do sistema — usa o estilo nativo do browser sem customização CSS, enquanto os outros campos de data estão padronizados | 🟡 Média |
...
| 70 | ESP-26 | Infraestrutura — Criação | Reservas criadas por papéis com permissão direta sempre iniciam como "Pendente" — mesmo em espaços sem responsável configurado. Comportamento esperado: se o espaço não tiver responsável no slot → auto-confirmar na criação; se tiver responsável → manter fluxo de aprovação. Backend tem `resolverResponsaveis()` e UI de config de responsáveis por espaço ✅ — falta usar o resultado para definir o status inicial | 🔴 Alta |
...
| 88 | ACO-11 | Ações Culturais — Header | Cabeçalho usa `h2.view-titulo` + `p.view-subtitulo` (13px) em vez de `h1.view-title` + `p.view-subtitle` (14px) — h2 é semântico incorreto para título principal de página; fragmenta o catálogo de classes de cabeçalho (3 padrões coexistindo no sistema) | 🟡 Média |
| 89 | ACO-12 | Ações Culturais — Formulário | Modal "Nova Ação" usa `class="input"` nos campos — 3ª variante de campo de form no sistema (além de `form-input` e `form-control`). Funciona mas fragmenta o DS | 🟡 Média |
...
| 103 | ACO-21 | Ações × Reuniões | Campo `run-acao-id` no formulário de Reunião é **texto livre** (placeholder "ACAO-001") — vínculo frágil, sujeito a erro de digitação. Deve ser select/autocomplete de ações ativas | 🔴 Alta |
| 104 | ACO-22 | Ações × Balcão | Campo `bl-acao-id` no formulário do Balcão é **texto livre** (placeholder "ACAO-001") — mesmo problema que ACO-21. Deve ser select de ações ativas | 🔴 Alta |
...
| 120 | ACO-07r | Ações — Contratações | Botão "+ Nova Contratação" no painel da Ação completamente inoperante — nenhuma resposta ao clicar. A aba carrega corretamente (estado vazio ✅), mas a ação principal está quebrada | 🔴 Alta |
| 121 | MAP-01 | **Mapa (ambos os editores)** — Mesclar | **BUG de algoritmo — afeta Mapa do Evento (Ações) E editor de mapa em Infraestrutura → Config → Mapa**: operação "Mesclar" transforma todas as formas selecionadas em um único grande quadrilátero (bounding box), destruindo os contornos individuais. Correto: (1) formas que se tocam → mesclar nas arestas/pontos de contato, preservando o contorno externo composto; (2) formas que não se tocam → criar bridge entre os dois pontos mais próximos, mantendo os contornos individuais intactos. Resultado deve ser polígono composto que respeita a geometria original, não uma caixa envolvente. Correção deve ser aplicada no núcleo compartilhado — ver ACO-10 | 🔴 Alta |
| 122 | MAP-02 | Mapa do Evento — Abertura | **BUG de sobreposição**: ao abrir o editor de mapa, a aba "Contratações" do painel da Ação permanece visível no plano de fundo — o editor não ocupa adequadamente a tela ou não oculta o conteúdo subjacente (provável problema de z-index ou de isolamento de camada) | 🔴 Alta |
| 123 | MAP-03 | Mapa do Evento — Fechamento | **BUG de navegação**: ao fechar o editor de mapa, o sistema navega para a view "Contratações" em vez de retornar ao painel da Ação. Instância confirmada de ACO-26 | 🔴 Alta |
| 125 | RECE-15 | RECE — Filtros | Datepicker na filter bar exibe "---------- de ----" — placeholder ou renderização quebrada; formato não segue o padrão dos demais inputs de data do sistema | 🟡 Média |
...
| 134 | BAL-05 | Balcão — Linguagem | Subtítulo "Gestão de demandas com SLA, versionamento e rastreabilidade" usa jargão técnico desnecessário na interface. Usuário final não precisa saber o que é SLA, versionamento ou rastreabilidade para usar o Balcão. Subtítulo deve ser substituído por linguagem orientada a uso: ex. "Solicite materiais e serviços ao setor de Comunicação" | 🟡 Média |
...
| 136 | BAL-06 | Balcão — Modal Dados | Campo "Ação vinculada (ID)" usa texto livre com placeholder "ACAO-001" — mesmo anti-padrão de ACO-21/ACO-22. Deve ser select/autocomplete por **nome** da ação a partir da lista de ações abertas. Nunca expor ID interno ao usuário | 🔴 Alta |
| 137 | BAL-07 | Balcão — Modal Dados | Campo "Executor (email)" é texto livre (placeholder "responsavel@ccbj.org.br") — deve ser autocomplete/select da base de usuários do sistema | 🔴 Alta |
| 138 | BAL-08 | Balcão — Modal Dados | Campo "Setor demandante" é texto livre (placeholder "Ex: NArTE") — deve ser dropdown com setores cadastrados na base | 🔴 Alta |
...
| 142 | BAL-12 | Balcão — Modal Dados | "⏱ SLA: 72h (3 dias) após submissão" expõe jargão técnico (SIS-08) e valor hardcoded. Substituir por linguagem de uso: "Prazo de atendimento: 3 dias úteis após o envio". Valor deve ser configurável por tipo de demanda no Admin | 🟡 Média |
...
| 169 | FIN-10 | Financeiro — Memória de Cálculo | Coluna "Subtotal" truncada no modo edição — largura insuficiente; modal read-only mostra valores completos | 🟡 Média |
...
| 172 | FIN-13 | Financeiro — Aditivos | Card "Valor Aditivado" exibe "—" em vez de "R$ 0,00" — inconsistência de formatação com os demais cards da aba | 🟡 Média |
| 173 | CON-01 | Contratações — Formulário | Campo SETOR SOLICITANTE é texto livre (placeholder "Ex: Produção") — mesmo anti-padrão sistêmico de CHV-06, EMP-03, TAR-02, ADM-03; deve ser dropdown de setores cadastrados | 🟡 Média |
...
| 181 | REU-02 | Reuniões — Modal Dados | "Convocado por (email)" é texto livre — anti-padrão sistêmico recorrente (TAR-02, CHV-05, ACO-14); deveria ser autocomplete/select de usuários | 🟡 Média |
| 182 | REU-03 | Reuniões — Modal Dados | Layout da aba Dados: Tipo ao lado de Data e Hora — mais coerente seria Tipo\|Local (qualitativo) e Data\|Duração (temporal) | 🔵 Baixa |
| 183 | REU-04 | Reuniões — Modal Pauta | Aba Pauta sem estado vazio de lista — quando nenhum item adicionado, área em branco sem mensagem | 🔵 Baixa |
| 184 | REU-05 | Reuniões — Modal Presença | Botões "+ Presente" e "+ Ausente Justif." com aparência idêntica — semântica oposta deveria ser visualmente diferenciada | 🟡 Média |
| 185 | REU-06 | Reuniões — Modal Ata | Botões usam emojis (💾 Salvar Rascunho, 🏛️ Submeter para Aprovação) — violação do padrão DS; usar Material Symbols | 🟡 Média |
| 186 | REU-07 | Reuniões — Modal Encaminhamentos | Layout horizontal com 3 campos + botão numa linha — campos truncam; reestruturar em 2 linhas | 🟡 Média |
| 187 | REU-08 | Reuniões — FSM | FSM da UI incompleto: filtro de status e ações dos cards cobrem apenas 5 dos 7+ estados do backend; `confirmada`, `ata_pendente`, `aprovada`, `arquivada` ausentes na UI | 🔴 Alta |
...
| 206 | FIN-14 | Financeiro — FSM de Contrato | FSM do contrato existe no backend (Ativo↔Suspenso→Encerrado) mas **sem botões dedicados de transição na UI** — a mudança de status acontece editando o formulário de contrato e alterando o select `#cf-status`. Sem: confirmação, campo de motivo/justificativa, notificação aos envolvidos, registro de quem encerrou e quando. Encerrar um contrato de R$1,6M por select é operacionalmente arriscado. Arquiteturalmente: deveria haver botão "Suspender" e "Encerrar" no card ou no painel de detalhes, com modal de confirmação + motivo | 🔴 Alta |
...
| 209 | FIN-16 | Financeiro — Pessoal (painel) | **Card "Custo Mensal" ausente nas métricas da aba Pessoal** — o painel exibe 5 cards (Salário Total, Encargos, Benefícios, Provisões, Custo Total/13 meses) mas não o Custo Mensal (R$ 12.994,14). O valor existe no cálculo ("CUSTO MENSAL" visível no modal "Editar Pessoal") mas não é exibido como card de métrica no painel — que é a informação mais comparável ao orçamento mensal do contrato | 🟡 Média |
| 210 | FIN-17 | Financeiro — Pessoal (cálculo) | **Benefícios Líquidos calculados incorretamente** — dados confirmados no modal "Editar Pessoal": Alimentação R$594,36 + VT R$0,00 + Saúde R$597,97 − Desconto Alimentação R$1,00 = **esperado R$1.191,33**; exibido: **R$525,74** (erro de ~R$666). Confirmado pelo usuário. A fórmula `_calcBeneficios()` não soma todos os componentes corretamente. Nota: este bug foi parcialmente documentado em correcoes.md (item 9) mas não registrado com ID no roteiro nem corrigido no commit 0e9317f | 🔴 Alta |
...
| 218 | ESC-11 | Escuta — Distribuição | Saturação por Dimensão exibe IDs numéricos (0, 1, 2…7) em vez dos nomes das dimensões (vigor, dedicação, absorção…) — instância de SIS-08 e do padrão de CON-03 | 🟡 Média |
...
| 223 | ESC-16 | Escuta — Modal | Título "Editar Pesquisa" exibido ao criar nova pesquisa — deveria ser "Nova Pesquisa" na criação | 🟡 Baixa |
...
| 94 | SIS-04 | Sistema Global — DS | Três classes de aba ativa coexistindo: `.ativa` (majoritário), `.active` (Aprovações, inglês), `.tab-ativa` (Dashboard/Escuta/Ponto/RH) — sem padrão único | 🟡 Média |
| 95 | SIS-05 | Sistema Global — DS | Três padrões de cabeçalho de view coexistindo (page-header+page-title, view-header+view-title, view-header+view-titulo) — sem convergência para padrão único; manutenção fragmentada | 🟡 Média |
...
| 2026-05-31 (sessão 13) | Reserva de Veículo (mod-34) — estrutura completa mapeada + decisões de domínio | Módulo nunca testado auditado pela primeira vez. Lista: card APROVADA com FSM operacional ✅; datas ISO (SIS-14), email em vez de nome (CAR-06/07). Agenda: grade mensal com reserva plotada ✅. Modal de detalhes: rota visual ✅; omite setor/passageiros/observação/ação (CAR-08). Formulário: setor integrado ✅; passageiros texto livre sem distinção interno/externo (CAR-09); sem paradas de rota (CAR-10); sem bloqueio frontend de passado (CAR-11); sem motorista (CAR-12); select de ação persiste em "Carregando" (CAR-04). Decisões de domínio confirmadas: (1) passageiros = internos + externos; (2) paradas intermediárias são gap funcional; (3) reservas passadas proibidas; (4) datas em pt-BR são regra sistêmica (SIS-14); (5) voucher Uber = novo fluxo esperado vinculado à rubrica de transporte (CAR-13). 13 novos problemas → total 256. |
...
### Estado atual: 256 problemas registrados
...
**Novos problemas s13:** CAR-02 a CAR-13 (12), SIS-14 (1) = **13 novos → total 256**
...
O formulário não tem campo de motorista. Perguntar quem conduz o veículo e quantos veículos compõem a frota.

> "O formulário de Reserva de Veículo não tem campo de motorista — quem dirige o carro? Existe um motorista fixo na equipe de Infraestrutura, ou o próprio solicitante conduz? E o CCBJ tem mais de um veículo?"

### Sequência após essa pergunta

1. **Reserva de Veículo** — motorista e frota ← **PRÓXIMA**

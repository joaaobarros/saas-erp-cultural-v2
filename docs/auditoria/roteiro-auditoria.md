...
### Estado atual: ~270 bugs registrados · Deploy @688 (GAS) · Firebase live

### O que foi feito nesta sessão (s40)

| Deploy | Fase | O que foi implementado |
|---|---|---|
| pendente | Fix: exclusão de tarefas | Botão "Excluir" no painel expansível de cada tarefa (cor error, alinhado à direita). `GAS.tarefas.excluir` mapeado para `ctrl_tarefas_excluir`. `confirm()` antes de deletar; lista recarregada após sucesso. |

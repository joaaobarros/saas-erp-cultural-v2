...
**Fase atual**: **Feat — Ações: Fases de projeto + cronograma visual + filtro "Minhas ações" — Deploy @874.** `acao_engine.gs`: campo `fases:[]` adicionado ao schema de Ação; funções `salvarFase()` e `excluirFase()` com auditoria e lock via repository. `acoes_controller.gs`: `ctrl_acoes_salvar_fase` e `ctrl_acoes_excluir_fase` com RBAC (coordenador+). `index.html`: (1) GAS namespace — `salvarFase`, `excluirFase`; (2) botão "Minhas ações" na filter bar — filtra por `responsavel` ou `equipe[]` do usuário logado; (3) nova tab "Fases" no painel de detalhe — CRUD completo de fases com cronograma visual (barras CSS proporcionais por datas) e pipeline em cards coloridos por status.

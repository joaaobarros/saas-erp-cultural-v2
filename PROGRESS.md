...
**Fase atual**: **Meu Perfil — fix carga automática + salvar + BtnGuard + Descartar (2026-06-10)** — Deploy @768. (1) `carregar(done)`: contador de geração (`_gen`) para ignorar callbacks stale de chamadas anteriores; auto-retry único após 7s se servidor não responder; `done()` aceito como parâmetro para integração com BtnGuard. (2) `aoAbrir()`: sempre recarrega ao abrir a aba (remove guard `_carregado`); passa `_retry=true` para habilitar o auto-retry da primeira carga. (3) `salvar(done)`: aceita e chama `done()` nos callbacks de sucesso e erro — corrige botão preso em "Salvando..." indefinidamente. (4) Botão Recarregar: add id `perfil-btn-recarregar`, BtnGuard.wrap com label "Carregando…", remove `data-bg-skip="1"`. (5) Botão Descartar: ícone trocado de `refresh` para `cancel`, também envolto em BtnGuard.

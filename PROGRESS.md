...
**Fase atual**: **UX — setor legível em todos os módulos (RH, Ponto, Admin, BI, Meu Perfil) — Deploy @834.** Helper global `_labelSetor(id)` adicionado (próximo a `fmtDataPtBR`): consulta `App.getBoot().setores` por `id` ou `nome`, retorna `label||nome` ou o próprio id se não encontrado. Aplicado em 8 pontos: tabela equipe RH, modal detalhes colaborador, cabeçalho período Ponto/Métricas, tabela "Por Setor" no Ponto, card de Risco CLT, tabela individual Ponto, tabela de usuários Admin, seção "Dados Profissionais" do Meu Perfil.

...
**Fase atual**: **Ponto — fix espelho + filtros por permissão + reprocessar jornadas (2026-06-09)** — Deploy @719. (1) Bug root-cause do espelho: `confirmarImportacao` chamava `JornadaEngine.processarImportacao()` que filtra brutos por `status=valido`; como todas as batidas eram `sem_cadastro` antes da criação de colaboradores, `jornadas.json` ficava vazio → espelho todo "Ausente". Fix: coletar `paresDias` no loop de saves e chamar `JornadaEngine.processarDia(orgId, colabId, data)` diretamente para cada par. (2) `ctrl_ponto_reprocessar_jornadas`: reprocessa jornadas a partir de `ponto_normalizado.json` existente sem re-importar; botão "Reprocessar Jornadas" na aba Sessões. (3) `ctrl_ponto_listar_colaboradores`: retorna colaboradores ativos + setores para os selects do filtro. (4) Filtros do espelho (setor + colaborador) visíveis apenas para superadmin/rh/gestor; usuários sem permissão veem só o próprio espelho sem filtros; mensagem adequada quando não há registros.

...
**Fase atual**: **Fase 15.3 — RH/DP completo: Folha CLT, PCCS arquitetural, Férias não oficial (2026-05-25)** — Deploy @219 ✅. Correções: (1) blur de backdrop persiste após fechar sidebar → Router.navegar() remove `.modal-simples,.rh-modal-overlay` antes de trocar view; (2) Folha CLT — form reconstruído com VT/VA/VR/PS configuraveis, máscara `_maskMoeda`/`_parseMoeda`, encargos corrigidos (35,6% = INSS20%+S.S+SAT6,6%+FGTS8%+PIS1%), exibição em 4 seções (III Remuneração · IV Encargos · V Benefícios · VI Provisões) + totais VII e VIII; (3) PCCS — `salvar()` corrigido para preservar `cargos`+`tabelaSalarial`, `aplicarReajuste()` atualiza ambos `cargo.tabela` e `tabelaSalarial`, frontend usa `_pccsGetSteps()` com fallback para `_PCCS_SAL_DEFAULT`, `abrirFormCargo()` carrega setores via `GAS.admin.listarSetores()` em `<select>` em vez de input livre; (4) Férias — bug de field names `inicio`/`dataInicio` corrigido no engine, form tem Tipo (Oficial / Não Oficial), ao selecionar Não Oficial aparecem campos `diasGozados` e saldo calculado em tempo real, lista tem coluna Tipo com badge Acordo; (5) Tests — `tests/backend/pccs-validacao.md` com 5 cenários executáveis no GAS Editor. **Próximo**: smoke-test browser → BtnGuard.auditar().

...
**Fase atual**: **Perfil Pessoal — sync 3-vias (2026-06-09)** — Deploy @759. Completado o triângulo de sincronização: (1) `ctrl_pessoas_meu_perfil_salvar` agora propaga `nomeApelido`, `pronomes`, `telefone`, `emailPessoal` e `fotoPerfil` para `usuarios_acesso.json` após salvar no colaborador; (2) `ctrl_acesso_editarPapel` propaga mudanças de `setor` feitas pelo Admin de volta para `colaboradores.json`; (3) `PerfilUI.salvar()` atualiza imediatamente `sidebar-user-name`, `topbar-email` e os campos do cabeçalho da view. O `ctrl_pessoas_salvar` (ficha RH) já propagava para `usuarios_acesso.json` — elo anterior já existia.

...
**Fase atual**: **Fix — getSetores() retornava array vazio para dados sem orgId — Deploy @865.** `config_service.gs → getSetores()`: quando a lista de `setores_config.json` existe mas o filtro `s.orgId === orgId` retorna vazio (dados legados salvos antes de `orgId` ser adicionado ao schema), a função agora tenta novamente sem o filtro de `orgId` antes de cair no fallback de `_defaultSetores()`. Resultado: dropdown de setor no `primeiro_acesso.html` (e em todo lugar que usa `getSetores()`) deixa de aparecer vazio para instalações com dados de setor legados.

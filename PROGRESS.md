...
**Fase atual**: **Fase 73 — Backend completo do módulo Estoque (2026-06-04)** — Deploy @561. 4 novos arquivos criados em `gas/src/modules/estoque/`: (1) `item_estoque_repository.gs` — MASTER.ItensEstoque (16 col) + MASTER.SaldoEstoque (8 col) + MASTER.MovimentacoesEstoque (append-only, 16 col) + depositos_config.json; (2) `solicitacao_material_repository.gs` — JSON canônico solicitacoes_material.json + índice ESPACOS.SolicitacoesMaterial; (3) `estoque_engine.gs` — FSM pendente→separada→finalizada, `LockService` em toda movimentação, `listarItens`, `salvarItem`, `registrarEntrada`, `registrarSaida`, `transferirEntreDepositos`, `novaSolicitacao`, `separarSolicitacao`, `entregarSolicitacao`, `cancelarSolicitacao`, `metricas`, `relatorioSaidas`, `relatorioEntradas`, `pipelineStatus`; (4) `estoque_controller.gs` — 15 funções `ctrl_estoque_*`. `core/setup.gs`: SCHEMA_ABAS expandido (+4 abas), `inicializarSistema()` com chamadas Fase 73, `fase73_estoque_prepararIndice()` + `setup_depositos_iniciais()` (seed: Almox. Central dep-01, Estoque Rápido Infra dep-02). `alertas_engine.gs`: +4 TIPOS (ESTOQUE_ITEM_CRITICO URGENTE, ESTOQUE_ITEM_BAIXO ATENCAO, ESTOQUE_PREVISTO_ACABAR ATENCAO, SOLICITACAO_SEM_SEPARACAO ATENCAO). **PRÓXIMO PASSO**: executar `fase73_estoque_prepararIndice()` no GAS Editor para criar as abas e seed dos depósitos. Depois: Fase 74 — EstoqueUI completa no frontend.

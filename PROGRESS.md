...
**Fase atual**: **Feat — Folha: Rescisão movida para Folha (sub-painel) + PS breakdown no verColab e holerite — Deploy @900.** `holerite_engine.gs`: objeto holerite agora expõe `psBruto`, `psDescEmpregado`, `psEmpresa` separadamente. `index.html`: (1) Aba "Rescisão" removida do nav principal; conteúdo movido para `rh-folha-painel-rescisao` dentro da aba Folha; novo botão "Rescisão" no sub-nav de Folha; `setSubAbaFolha('rescisao')` popula select de colaboradores; `setTab('rescisao')` redireciona para Folha→Rescisão. (2) `verColab` mostra PS com 3 linhas: total do plano, desconto do funcionário (k×%), custo empresa (k−l). (3) Holerite: seção "Benefícios" expandida com breakdown PS (total → desc. funcionário → custo empresa). (4) `_descPSPercCache` carregado de EncargosEngine em `aoAbrir()` para garantir percentual correto.

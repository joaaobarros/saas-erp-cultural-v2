...
**Fase atual**: **Feat — Dimensões em tempo real nos editores de mapa (infraestrutura e ações) — Deploy @921.** HUD flutuante (`position:fixed`, `pointer-events:none`, aparece só durante drag de resize, some no `mouseup`) adicionado em 3 editores: `mapa_acao_editor.html` (`MapaAcaoEditorUI`) — usa `_escala.pixPorM`; mostra `Lm × Am` em resize single (rect/polígono + círculo) e multi-resize (bbox do grupo); `mapa_editor.html` (`MapaEditorUI`) — novo parâmetro `escala` em `abrir()`, HUD ativo quando escala passada; `index.html / InfraConfigMapaUI` — usa `_escalaInfra.pixPorM`, chamado após `_resizeInline`. `InfraConfigMapaUI.getEscala()` exposto e passado de `EspacosUI._editarMapa()` → `MapaEditorUI.abrir()`. HUD limpo em `_onMouseUp`, `fechar()` e `_voltar()`. Sem poluição visual: sem escala configurada o HUD não aparece; some imediatamente ao soltar o mouse.

...
**Fase atual**: **Feat — Drag-and-drop para reordenar níveis no modal "Gerenciar Níveis do Mapa" — Deploy @926+.** `index.html / InfraConfigMapaUI`: nova função `_initNiveisDragDrop(overlay)` com event delegation no `<tbody id="cfg-niveis-tbody">`. Cada `<tr>` tem `draggable="true"` e alça `⠿` como primeira coluna. No `drop`: sincroniza inputs → reordena `overlay._niveisEdit` → re-renderiza tbody via `_rowHtml`. A flag `tbody._dragInited` evita listener duplicado quando `_abrirModalNiveis` é chamada repetidamente (add/remove). Linhas novas adicionadas por `_modalNivelAdicionar` cobertas pela delegation sem rebind. `_sincronizarInputs` preserva edições de texto/número feitas antes do drag.

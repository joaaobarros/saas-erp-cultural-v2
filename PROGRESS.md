...
**Fase atual**: **Fix Ponto — Indicador visual ⏳ + renomear Reverter→Excluir na aba Sessões (2026-06-09)** — Deploy @731. `_setBtnCarregando(sessaoId, label)` substitui o botão por "⏳ Cancelando…" / "⏳ Excluindo…" ao confirmar o modal, enquanto a chamada GAS está em andamento. Botão "Reverter" renomeado para "Excluir" (operação remove o histórico, não cria versão anterior).

...
**Fase atual**: **Fix — Rescisão: aviso de férias vencidas só aparece ao ultrapassar prazo legal + ícones Material Symbols — Deploy @902.** Info de férias só mostra mensagem quando `totalDobro > 0`; texto sem jargão técnico ("API", "Período N"); ícones emoji substituídos por `<span class="ms ms-sm">warning/report/error/info</span>` em toda a área de férias vencidas (rescisão, tabela de períodos e alertas). Fallback silencioso quando dados indisponíveis.

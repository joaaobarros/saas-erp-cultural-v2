...
**Fase atual**: **Fix — primeiro_acesso: fallback JS para dropdown de setor vazio — Deploy @867.** `acesso_service.gs`: adicionado `ctrl_acesso_getSetores()` (sem auth, setores são dados públicos da org). `primeiro_acesso.html`: `window.addEventListener('load')` detecta se `<select id="setor">` tem apenas o placeholder e, se sim, busca setores via `google.script.run.ctrl_acesso_getSetores()` e popula dinamicamente. Cobre todos os cenários em que o template server-side falha a passar setores (orgId mismatch, Drive permission, contexto de execução "Execute as: User"). Complementa o fix anterior do `getSetores()` em `config_service.gs`.

...
**Fase atual (s89b)**: **Fix — BI Demográfico: remove rótulo "★ epicentro" do tooltip de bairros; diferenciação visual por cor (âmbar) é suficiente.** `_tooltipBairro` simplificado (parâmetro `epicentro` removido); duas chamadas `.bindTooltip` atualizadas.

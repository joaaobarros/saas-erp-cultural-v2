...
**Fase atual**: **Fase 16.1 — Reservas: Manutenção + CCBJ Fechado todos os modos + Itens real-time + Migração V1 (2026-05-25)** — Deploy @231 ✅. Implementado: (1) `mapa_controller.gs`: emManutencao no status + itensFixos no retorno de `ctrl_mapa_statusEspacos`; (2) `admin_controller.gs`: `ctrl_admin_alternarManutencaoEspaco()` com RBAC admin/gestor/infraestrutura; (3) `migracao_itens_v1.gs` (novo): `migrar_itens_v1()` lê CCBJ_ESPACOS.Itens via `V1_ESPACOS_SHEET_ID` em Script Properties; (4) `almoxarifado_engine.gs`: `_parseItensVolantesStr()` global + `_horariosSobrepoem()` + `calcularDisponibilidadeItens()` com cruzamento de empréstimos, itensVolantes de reservas sobrepostas e itensFixos de outros espaços; (5) `reservas_controller.gs`: `ctrl_reservas_disponibilidadeItens(params)` com validação de sala/data/horário; (6) `index.html`: GAS bindings (alternarManutencao + disponibilidadeItens), `#res-admin-toolbar` sempre visível para admin/gestor (acima do modo-toggle), `#modal-manutencao-espaco` com select + badge de status + confirm, blocos itens fixos e volantes no form de reserva, chips verdes/vermelhos com re-validação ao mudar horário, salvar bloqueado com chips vermelhos; (7) `mapa_ui.html`: captura `_meuPapel` de App.getBoot, seção "Equipamentos Fixos" no painel lateral, botão "Manutenção/Liberar" admin-only, `_alternarManutencao()` com BtnGuard. **Próximo**: smoke-test browser: (a) trocar modo lista/agenda/diagrama → toolbar admin aparece; (b) "Manutenção de Espaço" → modal → confirmar → mapa muda cor; (c) "CCBJ Fechado" abre modal existente; (d) form reserva: selecionar sala/data/hora → itens carregam com disponibilidade; (e) chip vermelho bloqueia salvar.

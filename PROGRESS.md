...
**Fase atual**: **Fase 15.4 — PCCS: tabela salarial editável completa (2026-05-25)** — Deploy @221 ✅. Implementado: (1) `PCCSRepository.salvarTabelaSalarial(pccsId, tabelaSalarial, parametros, email)` no repositório; (2) `ctrl_pccs_salvarTabela(pccsId, tabelaSalarial, parametros)` no controller; (3) `GAS.rh.salvarTabela` binding no frontend; (4) `_abrirModalRhLargo` helper para modais de até 900px; (5) Modal completo de edição da tabela salarial: seção Parâmetros (ano, pisos, crescimento/step) + tabela FIXA (18 classes) + tabela ORIENTADOR (12 classes), cada célula editável, botão ↺ por linha e "Recalcular todos" global; (6) Botão "Tabela Salarial" no header de cada plano PCCS; (7) Pré-preenchido com dados IDM 2025 via fallback `_PCCS_SAL_DEFAULT`. **Próximo**: smoke-test browser: PCCS → Tabela Salarial → editar valor → Salvar → verificar no JSON e no form Cargo.

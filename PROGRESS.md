...
**Fase atual**: **Fase 70.1 — fix permissões: AGENTES + VOLUNTARIOS no engine v2 (2026-06-04)** — Deploy @533. **PERM-01 CORRIGIDO** — `permissoes_v2_engine.gs`: `AGENTES` e `VOLUNTARIOS` adicionados a `_MODULOS`, `_MATRIZ` (todos os 9 papéis) e `_MODULO_LABELS`. Eles não existiam no engine, então `permissoesModulos` nunca retornava essas chaves → menu nunca filtrava Agentes e Voluntários mesmo em simulação com tudo desmarcado. Agora ambos aparecem na matriz do modal de simulação e são filtrados corretamente. `index.html`: `aprovacoes` recebe `modulo:null` explícito (antes ausente — estava correto por design mas impreciso). Colaborador: AGENTES=_p(1,0,0), VOLUNTARIOS=_p(0,0,0).

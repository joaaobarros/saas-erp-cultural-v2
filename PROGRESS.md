...
**Fase atual**: **Fase 74 — Frontend EstoqueUI completo (2026-06-05)** — Deploy @563. `gas/src/frontend/index.html` (+951 linhas): (1) **GAS.estoque** namespace com 15 bindings (`listarItens`, `salvarItem`, `metricas`, `saldoItem`, `registrarEntrada`, `registrarSaida`, `transferir`, `pipeline`, `novaSolicitacao`, `separarSolicitacao`, `entregarSolicitacao`, `cancelarSolicitacao`, `listarSolicitacoes`, `relatorioSaidas`, `relatorioEntradas`); (2) **Tab "Estoque"** adicionada na barra de tabs do módulo Infraestrutura (entre Patrimônio e Configurações); (3) **`<div id="esp-tab-estoque">`** com 5 sub-abas: Itens (catálogo em grid cards, filtros busca/categoria/situação, ação Editar + Entrada), Estoque (saldo por depósito/local, cards com qtd real/alocada/disponível, ação Transferir), Solicitações (pendentes + separadas com ações Separar/Entregar/Cancelar + botão Nova Solicitação), Entradas (filtro por período, tabela mensal), Relatórios (saídas sintéticas por setor + entradas mensais); (4) **`EstoqueUI`** IIFE com ~500 linhas: `aoAbrir`, `abrirSubTab`, métricas no topo (MetricsToggle), 5 funções de carga de sub-aba, modais (Item, Entrada, Transferência, Nova Solicitação com carrinho interativo). **PRÓXIMO PASSO**: Fase 75 — Pipeline visual + Alertas inteligentes de estoque. Pré-requisito: executar `fase73_estoque_prepararIndice()` no GAS Editor se ainda não foi feito.

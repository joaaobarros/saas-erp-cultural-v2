...
**Fase atual**: **Pulse — clima por setor (2026-06-06)** — Deploy @631. PUL-07 CORRIGIDO: (1) `escuta_pulse.gs/_calcPorSetor`: usa `u.setor || u.setorDesejado` como fallback — aprovações rápidas que deixavam setor vazio agora caem no setorDesejado do colaborador; usuários sem nenhum setor são excluídos em vez de agrupados em 'Sem setor'. (2) `index.html/_carregarPendentes`: botão "Aprovar" adiciona `data-setor` com o `setorDesejado`; `aprovar(email, setor)` repassa o setor ao `aprovarAcesso`; novas aprovações já definem setor corretamente desde o início.

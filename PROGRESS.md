...
**Fase atual**: **Fix — Férias/Rescisão: saldoOficial sem acordos + Encargos: botão "Aplicar tabela" travado eternamente — pendente deploy.** (1) `pessoas_engine.gs/resumoFeriasPorPeriodo`: adicionado `saldoOficial` (sempre usa datas oficiais `dataInicio`/`dataFim`, ignora `acordo.diasEfetivosGozados`) ao lado de `saldo` (preservado para a view de gestão de férias). Frontend de rescisão usa `p.saldoOficial` com fallback para `p.saldo`. (2) `index.html/EncargosUI.aplicarOficial`: função agora aceita parâmetro `done` do BtnGuard e o chama imediatamente após abrir o modal de confirmação — elimina travamento eterno do botão "Aplicando tabela oficial…".

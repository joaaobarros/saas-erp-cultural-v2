...
**Fase atual**: **Auditoria HUB-04/05/09 (2026-06-06)** — Deploy @642. HUB-04 CORRIGIDO: empty state da aba "Meu Time" agora orienta o usuário com CTA (criar tarefas em Tarefas / encaminhamentos em Reuniões). HUB-05 CORRIGIDO: unidade '(h)' movida para o label do card de Tempo Médio; valor de Dias Médios sem 'd' embutido. HUB-09 CORRIGIDO: `ctrl_taskhub_meu_time` enriquece cada entrada com nome via `AcessoService.listarUsuarios()` (mapa bulk para evitar N+1); frontend exibe `p.nome || p.email` com `title` mostrando o email completo. HUB-01/02/06 verificados: MetricsToggle já presente na aba Produtividade; estado vazio usa Material Symbol não emoji; label "Meu Centro" em todos os pontos visíveis.

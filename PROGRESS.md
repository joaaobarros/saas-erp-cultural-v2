...
**Fase atual**: **s16 Fases 23-26 — RH-01/ESP-22/SIS-01(alm+sol)/ESP-01 (2026-06-01)** — Deploy @389 ✅. Corrigidos: (23) RH-01 — `_carregarMetricas` em RhUI reescrita: usa estrutura correta `stat-icon+stat-info+stat-valor+stat-label`, renderiza zeros mesmo em falha do backend (erro silencioso eliminado); (24) ESP-22 — auto-conclusão de reservas `em_uso` atrasadas: novo `ctrl_reservas_concluir_atrasadas` conclui silenciosamente reservas cujo `horaTermino + 15 min` já passou; chamado em `_executar` antes do load da lista; (25) SIS-01 (AlmoxUI + SolicitacoesUI) — `_devolver` em AlmoxUI e `devolver` em SolicitacoesUI substituem `prompt()` por `_abrirModalSimples` com textarea de observações e botão Confirmar/Cancelar; `_confirmarDevAlmox`/`_confirmarDevSol` exportadas; (26) ESP-01 — coluna de espaços no Diagrama de Reservas explicitamente marcada com `position:sticky;left:0;z-index:2` no inline style (garantindo prevalência sobre qualquer cascata CSS).

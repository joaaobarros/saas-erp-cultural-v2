...
**Fase atual**: **Fase 16.2 — Encargos Trabalhistas automáticos + edição manual auditada (2026-05-26)** — Deploy @237 ✅. Implementado: (1) `encargos_repository.gs` (novo): CRUD de `encargos_trabalhistas.json`; alíquotas, salário mínimo, tabela INSS progressiva, tabela IRRF com fonte `oficial`/`manual` por item; override manual preservado ao aplicar tabelas oficiais; histórico das últimas 100 alterações; (2) `encargos_engine.gs` (novo): `TABELAS_OFICIAIS` com catálogo 2024 e 2025 (Decreto 12.302/2024 + Portaria MPS 1.383/2024); `atualizarParaAno()`, `verificarNecessidadeAtualizacao()`, `executarAtualizacaoAutomatica()` (idempotente), `getParametrosRHComEncargos()` que funde config_org + encargos.json; (3) `encargos_controller.gs` (novo): `ctrl_encargos_listar/historico/preview_oficial/salvar_aliquota/salvar_salario_minimo/salvar_tabela_inss/salvar_tabela_irrf/restaurar_oficial/aplicar_oficial`; RBAC escrita = rh/admin/superadmin; (4) `ponto_engine.gs`: `_getParametrosRH` passa a usar `EncargosEngine.getParametrosRHComEncargos()` com fallback para `SistemaConfigService`; `calcularCustoCLT` usa alíquotas lidas do JSON (inssPatronalAliq, satAliq, sistemaSAliq, pisAliq); retorno inclui `sat`, `sistemaS`, `pisPasep` separados e `_aliq*` para exibição dinâmica; (5) `setup.gs`: `EncargosRepository.inicializar()` em `inicializarSistema()`; `fase16_encargos_prepararIndice()` global; `triggerAtualizacaoAnualEncargos()` (dia 1 de cada mês — só aplica se nova tabela disponível); `_instalarTriggerAnualEncargos()` com remoção de duplicatas; (6) `index.html`: aba "Encargos" na tab-bar do RH; `GAS.encargos.*` namespace (9 bindings); `EncargosUI` completo: tabela de alíquotas editável por item, salário mínimo editável, modal tabela INSS progressiva, modal tabela IRRF, pré-visualização da tabela oficial, histórico de alterações, badges oficial/manual; seção encargos na folha CLT atualizada com alíquotas dinâmicas. **Próximo**: smoke-test browser: RH → aba Encargos → verifica tabela 2025 carregada → edita uma alíquota (ex: SAT 1,5%) com justificativa → Salvar → badge manual aparece → botão ↩ Oficial restaura → Aplicar tabela oficial 2025 → itens manuais preservados.
...
| 13 | Produto Pronto — Exportações Institucionais | 🔵 Em andamento | Alta | 2026-05-24 | — |

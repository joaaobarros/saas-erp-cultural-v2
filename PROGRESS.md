...
**Fase atual**: **Fix 72.2 — Polígonos livres no MapaUI (2026-06-04)** — Deploy pendente. `gas/src/shared/mapa_ui.html`: adicionado `case 'livre'` em `_criarFormaEl()` — quando `cfg.forma === 'livre'` e `cfg.pts` com ≥ 3 pontos, cria `<polygon points="...">` com as coordenadas relativas ao centro, idêntico ao comportamento já correto do `InfraConfigMapaUI`. Antes, o `switch` sem este case caía no `default` (triângulo hardcoded), fazendo todos os espaços editados com vértices personalizados aparecerem como triângulos no mapa de reservas.

**Fase anterior**: **Fase 72.1 — Seed + migração real de itens do almoxarifado V1→V2 (2026-06-04)** — Sem deploy (backend only). `core/setup.gs`: seed de 24 itens padrão CCBJ + 3 funções de migração real do V1 (`fase72_migrar_itens_v1_automatico`, `fase72_migrar_itens_v1_por_id`, `fase72_itens_almoxarifado_seed`). EXECUTAR: `fase72_migrar_itens_v1_automatico()` no GAS Editor do V2.

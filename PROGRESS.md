...
**Fase atual (s93)**: **UX — Reserva de Veículo: Editar Rota com picker de mapa, gestão de paradas, tempo de espera e otimização de rota — Deploy @988 (pendente).** `index.html`: painel "Editar Rota" do modal de detalhes (aprovadores) completamente reescrito — (1) local de chegada com botão `place` que abre o Leaflet picker (contexto `det-chegada`); (2) lista de paradas com add/remove dinâmico, botão de mapa por parada (contexto `det-parada`) e campo "Tempo de espera (min)" step=5 por parada; (3) botão "Rota eficiente" — nearest-neighbor TSP: geocodifica paradas sem coordenadas via `ctrl_carro_geocode` e reordena as paradas pela rota mais curta a partir da origem; (4) botão "Salvar rota" envia `{localChegada, coordChegada, paradas:[{local,lat,lng,tempoParadaMin}]}`; (5) variáveis de estado `_mapaContexto`, `_editRotaId`, `_editRotaParadas`, `_editRotaChegada`, `_editRotaOrigem`; `confirmarMapaPicker` estendido com desvio por contexto. `reserva_carro_repository.gs`: comentário de schema atualizado com `tempoParadaMin`.

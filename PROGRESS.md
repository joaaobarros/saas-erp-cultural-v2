...
**Fase atual**: **Fase 15 completa (2026-05-24)** — Banco de Dados expandido: `_garantirEstruturaDrive()` cria hierarquia Drive (`CCBJ — ERP/Dados JSON/` + `CCBJ — ERP/Planilhas/`) e move todos os arquivos; `ctrl_admin_obterInfoBancoDados()` retorna URLs de pasta ERP, pasta JSON, pasta Planilhas e 11 planilhas com label+ícone+descrição; card "Banco de Dados" redesenhado (3 botões Drive + grid de planilhas com link individual); `GAS.admin.obterInfoBancoDados` binding; `AdminDadosUI.carregar()` chamado no `aoAbrir` do módulo admin ✅ · Deploy @201 ✅. **Próximo**: abrir URL produção → Administração → Banco de Dados → verificar 3 botões Drive + 11 cards de planilhas → BtnGuard.auditar() → Fase 13.2 SNIIC ou próxima feature.

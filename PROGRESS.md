...
**Fase atual**: **RH — Campo pai/mãe em Meu Perfil e Ficha Colaborador — Deploy @819.** Adiciona `ePaiMae` (Não informado/Não/Sim), `papelParental` (checkboxes Mãe/Pai/Responsável) e `numFilhos` (número). Painel condicional aparece só quando "Sim". Usado para datas comemorativas (Dia das Mães, Dia dos Pais) e homenagens. Backend: 3 campos novos em `_CAMPOS_PROTEGIDOS` + `_PERFIL_CAMPOS_EDITAVEIS`. Frontend: bloco idêntico em Meu Perfil e Ficha RH com populate/save completos.

...
**Fase atual**: **s17 Fases 53+54 (2026-06-03)** — Deploy @456. (53) SIS-03/04/05 CORRIGIDOS via CSS aliases: `.btn-primary,.btn-primario` e `.btn-secondary,.btn-secundario` unificados; `.tab-btn.active` alias de `.tab-btn.ativa`; `.page-title,.view-title` e `.page-subtitle,.view-subtitle` fundidos. REU-08 investigado → FALSO POSITIVO (5 estados do backend já cobertos). BAL-06 → JÁ CORRIGIDO por ACO-22. (54) ACO-28 CORRIGIDO: ao clicar "Concluir" em ação `em_execucao`, modal de encerramento abre com campos: público atingido (number), realizações (textarea), observações (textarea), comprovações (URL+desc com chips add/remove); ao confirmar → `acao_engine.mudarStatus` salva `encerramento` no objeto da ação; funções `_addComprovacaoEnc` e `_confirmarEncerramento` exportadas em AcoesUI; backend: `acao_engine.gs` aceita param `encerramento`, `acoes_controller.gs` repassa `params.encerramento`.

...
**Fase atual**: **Fase 14 completa (2026-05-24)** — View unificada `view-rh` com 10 abas (EQUIPE · PCCS · HISTÓRICO · AVALIAÇÕES · ESCALAS · DOCUMENTOS · FÉRIAS · FOLHA · RESCISÃO · INDICADORES) ✅ · `RhUI` IIFE completo (~600 linhas) com lazy-load por aba, BtnGuard em todos os botões async ✅ · Helpers globais `_abrirModalSimples`/`_abrirModalConfirmar` (fix bug scope MapaAcaoUI) ✅ · `GAS.rh.*` com 18 novos bindings (histórico, avaliações, escalas, férias) ✅ · confirm()/prompt() nativos substituídos por modais em AfastamentosUI e OcorrenciasUI ✅ · Deploy @198 ✅. **Próximo**: smoke-test browser → abrir URL produção → verificar as 10 abas → BtnGuard.auditar() → Fase 13.2 SNIIC ou Fase 13.3 multi-org.

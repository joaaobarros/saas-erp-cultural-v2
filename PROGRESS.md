...
**Fase atual (s94)**: **Feat — Datas Comemorativas: toggle ativo/inativo; Contratos: status Cancelado com FSM e badge — Deploy @992.** `index.html`: (1) `DatasComemorativasAdmin` — botão "restaurar"/"excluir" substituído por toggle `toggle_on`/`toggle_off` (Material Icons); chama `GAS.admin.toggleDataComemorativa(id, novoAtivo, ...)` com `BtnGuard.wrap`; `_STATUS_LABEL` e `_badgeStatus` no `ContratosUI` ganham entrada `Cancelado → badge-cancelado`; (2) `GAS.admin.toggleDataComemorativa` adicionado ao namespace. `admin_controller.gs`: `ctrl_admin_toggleDataComemorativa(id, ativo)` delegando ao service. `config_admin_service.gs`: `toggleDataComemorativa(id, ativo)` — altera apenas campo `ativo` no JSON sem tocar outros campos; `excluirDataComemorativa` reescrito como alias de `toggleDataComemorativa(id, false)`; `toggleDataComemorativa` exportado no return. `contratos_engine.gs`: `STATUS_CONTRATO.CANCELADO` adicionado ao enum; `_TRANSICOES_CONTRATO` aceita transição Ativo→Cancelado e Suspenso→Cancelado; `excluir()` aceita status Cancelado (além de Encerrado); métricas incluem `cancelados`; `listarParaVinculo` usa constantes corretas.

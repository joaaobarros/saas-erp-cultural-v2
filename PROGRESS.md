...
**Fase atual**: **Fase 16.3 — Dashboard RH/Folha (2026-05-26)** — Deploy @249 ✅. Implementado: (1) `rh_dashboard_controller.gs` CRIADO — `ctrl_rh_dashboard_folha(params)`: agrega custo estimado de todos os colaboradores ativos por vínculo (CLT→PontoEngine.calcularCustoCLT, PJ→salário direto, bolsista→+8%); retorna resumo (totalMensal, totalAtivos, custoMedio, projecaoAnual, mesesRestantes), breakdown por vínculo, porSetor, historico12m e itens ordenados por custo; `ctrl_rh_dashboard_ponto(params)` — modo `heatmap` (todos colaboradores × 6 meses com banco de horas classificado em ok/medio/alto/critico/negativo), modo `calendario` (espelho diário de um colaborador com status presente/parcial/ausente/fimSemana); `ctrl_rh_dashboard_turnover(params)` — tendência 12 meses com entradas/saídas/headcounts/taxas + mediaTurnover + altoRisco (>15%). (2) `index.html` — 3 novos bindings `GAS.rh.dashboardFolha/dashboardPonto/dashboardTurnover`; CSS: 12 blocos de estilo (rh-dash-strip/card, rh-folha-sub-nav, rh-dash-bar, rh-hist-chart, bh-grid/cell, ponto-cal/dia, tv-bar/taxa-strip); HTML Tab Folha: sub-nav "Dashboard | Calculadora" + painel `#rh-dash-folha-conteudo` (stats strip 4 cards, grid breakdown×histórico, tabela top colaboradores); HTML Tab Indicadores: 4 seções (Turnover 12m, Heatmap BH, Calendário Ponto, Turnover Mês Específico); JS RhUI: `setSubAbaFolha`, `carregarDashboardFolha`, `carregarTurnover12m`, `carregarHeatmapBH`, `carregarCalendarioPonto`, `_inicializarCalendarioPonto`; `setTab` atualizado (folha→carregarDashboardFolha, indicadores→+carregarTurnover12m+_inicializarCalendarioPonto). **Auditoria CLAUDE.md**: BtnGuard nos 5 botões assíncronos (rh-btn-dash-folha×2, rh-btn-turnover, rh-btn-heatmap, rh-btn-cal) ✅ | GAS.* namespace 100% (3 novos bindings = 3 controllers) ✅ | CSS todos as classes usadas definidas ✅ | nenhum modal novo ✅ | FsmGuardian n/a (somente leitura) ✅ | nenhum prompt()/confirm() ✅. **Próximo**: smoke-test browser: (a) RH/DP → Folha → sub-aba Dashboard → stats aparecem + breakdown por vínculo + histórico 12m; (b) RH/DP → Indicadores → Turnover 12m carrega; Heatmap BH carrega; Calendário → select colaborador → mês atual → Carregar → grid de dias aparece; (c) F12 → zero erros | BtnGuard.auditar() → "✅ todos protegidos".

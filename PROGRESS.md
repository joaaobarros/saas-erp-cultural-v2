...
**Fase atual**: **Auditoria SIDEBAR-04 — Consolidação seção MEMÓRIA (2026-06-06)** — Deploy @600. 4 itens do menu (Agentes, Acervo, Voluntários, Parcerias) substituídos por 1 item "Memória Institucional" com tab-bar interna. `view-memoria` criada com painel para cada sub-módulo (`mem-panel-agentes/acervo/voluntarios/parcerias`); `MemoriaUI` IIFE gerencia as abas e delega `aoAbrir()` para cada sub-UI. `_MODULOS_MENU` reduzido de 4 entradas na seção MEMÓRIA para 1. Router `_construirMenu` atualizado; Router.registrar internos dos 4 sub-módulos removidos. Sidebar total: ~17 itens visíveis (redução de 3).

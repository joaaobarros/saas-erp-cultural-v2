...
**Fase atual**: **Auditoria SIS-13 — campo "responsável" texto livre (2026-06-06)** — Deploy @596. 5 campos convertidos de `<input type="text/email">` para `<select>` populado por `_carregarSelectUsuariosHelper`: (1) `tf-responsavel` (Tarefas quick form) — `aoAbrir()` chama helper pré-selecionando email do usuário logado; (2) `acao-responsavel` (Ações modal) — `abrirForm()` chama helper com email do responsável existente ou email do usuário logado; (3) `obj-responsavel` (Estratégia/Objetivos) — `abrirForm()` chama helper nos branches editar e novo; (4) `pat-modal-resp` (Patrimônio, modal dinâmico) — select gerado na string HTML + helper chamado após `_abrirModalSimples`; (5) `run-enc-resp-input` (Reuniões encaminhamentos) — helper adicionado no `abrirForm`. Campo `par-resp` (Parcerias) mantido como texto — contato externo, não usuário do sistema.

...
**Fase atual (s92)**: **Feat — Datas Comemorativas: calendário expandido com diversidade e direitos humanos; animação Papai Noel no Natal — Deploy pendente.** `index.html`: (1) 10 novas datas — Mulheres (8/3), Indígenas BR (19/4), Abolição BR (13/5), Dia da África (25/5), LGBTQIA+ (28/6), Indígenas ONU (9/8), Consciência Negra (20/11), Eliminação Violência contra Mulheres (25/11), Direitos Humanos (10/12) + Reveillon; `_DATAS_COMEMORATIVAS` agora usa campo `id` (alinhado com backend); (2) 5 novas classes CSS — `.dc-consciencia`, `.dc-africa`, `.dc-indigena`, `.dc-mulheres`, `.dc-arco-iris`; (3) novo motion `papai_noel` em `_DC_MOTION_CLS` — neve de fundo (70 flocos) + Papai Noel emoji (`🦌🦌🦌🎅🛷`) atravessando a tela com animação de balançar; Natal migrado de `motion:'neve'` para `motion:'papai_noel'`. `config_admin_service.gs`: `_DATAS_COMEMORATIVAS_DEFAULT` expandido de 10 para 19 datas, organizadas em 4 grupos (Cívicas, Negritude/Diáspora, Povos Originários, Direitos Humanos).

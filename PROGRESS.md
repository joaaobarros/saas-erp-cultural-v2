...
- **`_onLadoChange(idx, val)`**: ao alterar um input, calcula o ponto médio e o vetor unitário da aresta; move ambos os vértices simetricamente a partir do centro para atingir o novo comprimento. Chama `_atualizarHandles()` + `_atualizarLadosInputs()`.
- **`_atualizarLadosInputs()`**: sincroniza todos os inputs de lado com as posições atuais dos vértices (chamada por `_atualizarInputsInline` que é invocada após cada drag de vértice via `_atualizarHandles`).
- Exposta no objeto público: `_onLadoChange`.

**Fase anterior (s155c)**: **feat — Editor de espaços: painel lateral "Dimensões reais" — inputs editáveis por lado, altura e diâmetro.** Deploy @1125.
...
- **8 novas figuras**: Triângulo, Hexágono, Pentágono, Estrela, Balão de fala (`callout`), Marcador (pincel largo semitransparente), Pincel (largura variável por velocidade).
- **Estilo de linha**: 3 botões na barra inferior — Sólida / Tracejada / Pontilhada — aplicados a line, arrow, connector.
- **14 novos templates** em 4 categorias: Golden Circle, OKR, Matriz Eisenhower, Retrospectiva, Business Model Canvas, Persona de Usuário, User Flow, Wireframe Web, Mapa de Stakeholders, Jornada do Colaborador, Funil de Comunicação, Organograma.
- **Vincular Tarefa**: busca e insere card de tarefa do módulo Pessoal com status colorido.
- **Vincular Pessoa**: insere card com nome/cargo/módulo editáveis.
- **Dados do Sistema**: painel lateral para inserir widgets de Quadro, Ação, Reunião, Resumo de Tarefas ou Indicador.

**Fase anterior (s140)**: **feat — Estúdio de Análises v3: cruzamento livre de datasets, modo estático/ao vivo, Meu Painel personalizável, controle de visibilidade, filtragem por dono — Deploy @1071.**
...
- **Carregamento errado** (`abrirTab`, linha de lazy-load): ao clicar em "Patrimônio" era chamado `EstoqueUI.abrirSubTab('patrimonio')` em vez de `AtivosUI.carregar()`. Corrigido para chamar o controlador correto: `AtivosUI.carregar()` → exibe `esp-tab-ativos` com formulário e lista de bens patrimoniais.
- **Resultado**: "Patrimônio" → `AtivosUI` (bens/tombamento/localização); "Estoque" → `EstoqueUI` (almoxarifado/saldo/movimentações). Duas abas distintas, como esperado.

**Fase anterior (s125)**: **Fix — BI Demográfico: heat overlay endereço exato + visibilidade mínima garantida — Deploy @1037.**
...
- **Categorias de rubrica Lei Rouanet (Financeiro)**: rubricas só tinham `categoria` custeio/investimento (campo usado pelo SALIC, não alterado). Adicionado campo **novo e separado** `categoriaRouanet` (pessoal/custeio/capital/divulgação/captação/administração, vindo de `config_org.json tiposRubrica`) ao formulário de Item de Despesa (`index.html` — `cd-rub-categoria-rouanet`, `_tiposRubricaOpts()`). Exposto via boot (`boot_service.gs` → `_boot.tiposRubrica`, mesmo padrão de `tiposAfastamento`/`tiposOcorrencia`). Persistência sem mudança no repositório (objeto da rubrica já aceita campos arbitrários). Campo também adicionado ao XML do SALIC (`exportacao_engine.gs`).
- **Aba "Conformidade" (Dashboard)**: completa o que ficou pendente na sessão s115 (card de limites Lei Rouanet, então fora de escopo por falta de categorização). Novo `ContratosEngine.conformidadeRouanet(orgId)` soma rubricas por `categoriaRouanet` em todos os contratos `modalidade='lei_rouanet'`, compara contra `tiposRubrica[].limitePercentual` (divulgação 20%/captação 10%/administração 15%) e sinaliza alerta visual quando o percentual real ultrapassa o limite. Rubricas sem `categoriaRouanet` aparecem como "não classificado" e **não entram no cálculo** — nunca adivinhadas por nome, para não gerar número que engane prestação de contas real. Novo `ctrl_dashboard_conformidade`.
- Usuário também pediu para aprofundar as demais abas do Dashboard (Operacional/Financeiro/Estratégico/Estoque/Alertas), apontando que hoje só repetem as métricas já visíveis em cada módulo — isso fica para a próxima etapa desta mesma sessão, após alinhar com o usuário o que "mais profundo" significa concretamente por aba (trends temporais? drill-down? comparação de período?), para não construir na direção errada como já aconteceu 2x nesta sessão (Lei Rouanet sem categoria, painel de exportações duplicado).

**Pendente de confirmação do usuário**: smoke test do mapa por cidade (BI Demográfico → Cidades) e da aba Conformidade do Dashboard (só aparece conteúdo se houver contrato `modalidade='lei_rouanet'` com rubricas já classificadas — contratos antigos aparecerão com tudo em "não classificado" até serem editados).

**Fase anterior (s115)**: **Feat — Dashboard: aba "Alertas" operacionais (banco de horas, férias, ativos, almoxarifado) — Deploy @1033.**
...
| 2026-06-24 | s135 Quadros v2 | **Napkin + Miro + ferramentas canvas + cross-module sem quebra de fluxo** (Deploy @1061): `index.html` — QuadrosUI: campos `tipo/urlExterna/napkinPrompt/categoria` no form; Miro: live-embed URL automático; Napkin: URL por prompt; 12 categorias visuais dinâmicas por tipo (`_CATS_BY_TIPO`). Ferramentas: Sticky Note (7 cores, drop-shadow, word-wrap), Frame/Slide (borda roxa tracejada, label via `_modalInput`), Tabela (grid configurável). Borracha: hit-detection por closest-point-on-bbox + preview círculo vermelho. Apresentação: `apresentarCanvas()` (dark overlay pan/zoom/ESC) + `apresentarExterno()` (iframe fullscreen). Integração Ações+Reuniões: aba "Quadros" no painel da ação e no modal da reunião; `_novoQuadroAcao`/`_novoQuadroReuniao` abrem `#qdro-modal` (z-index:1100) sobre o contexto pai sem fechar. `setAfterSaveCb()` exposto em `QuadrosUI` — callback pós-save atualiza lista da aba correta sem navegar. | Smoke-test: Ações → painel → aba Quadros → Novo Quadro (modal abre sobre painel, painel permanece); Reuniões → modal → aba Quadros → Novo Quadro (modal sobre modal) |
| 2026-06-16 | s99 Fix badge setor | **Badge setor: cores únicas, sem repetição** (Deploy @1005): `_SETOR_PALETA` expandida de 10 → 14 cores distintas (violet, blue, green, yellow, rose, sky, pink, emerald, orange, purple, amber, teal, fuchsia, indigo); atribuição sequencial via `_setorCorNext++` em vez de hash (garante cores únicas enquanto ≤ 14 setores na mesma view); bug `!_setorCorIdx[id]` falso-positivo quando hash resultava em índice `0` corrigido para `=== undefined`. | Financeiro > Execução > agrupamento Setor: os 7 setores devem ter cores distintas (antes: Comunicação, NArTE, Administrativo e Escola de Cultura e Artes todos recebiam rosa). |
...
| 2026-06-26 | s156 Config Mapa — Lados | **Seção "Lados (m)" no painel de edição de espaços polígonos** (Deploy @1126): `InfraConfigMapaUI` (`index.html`) — `_ladosBlock`: grid 2 colunas com input por aresta (`cfged-lado-N`) numerados `1→2`…`N→1`, valor em metros, visível apenas para `forma==='livre'` com escala configurada. `_onLadoChange(idx, val)`: escala a aresta simetricamente a partir do ponto médio (vetor unitário × nova meia-largura em px), sem alterar direção ou centro do lado. `_atualizarLadosInputs()`: sincroniza todos os inputs ao arrastar vértices (`_atualizarInputsInline` chama automaticamente). Exposta em `InfraConfigMapaUI._onLadoChange`. | Smoke-test: Infraestrutura → Config → Mapa → editar espaço polígono com escala ativa → painel "Lados (m)" aparece → alterar valor → forma ajusta no mapa |

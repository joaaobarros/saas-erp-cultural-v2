...
**Fase atual**: **Fase 13.1 completa (2026-05-24)** — Exportação SALIC (XML completo: Proponente, Projeto, PlanoAplicação metas→atividades→rubricas, PessoalEquipe, Resumo, campo PRONAC no contrato, botão SALIC no detalhe) ✅ · Exportação PNAB — Lei Aldir Blanc (4 CSVs: Espaços, Agentes, Ações, Financeiro, tab Exportações no Financeiro, ExportacoesUI) ✅. **Próximo**: `clasp login` → `clasp push && clasp deploy --deploymentId AKfycb... --description "Fase 13.1"` → smoke-test browser (Financeiro → Exportações → SALIC XML + PNAB CSVs). Depois: **Fase 13.2 — SNIIC** e **Fase 13.3 — Provisionamento multi-org**.

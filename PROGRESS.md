...
**Fase atual**: **Fase 13.2 — Exportação SNIIC completa (2026-05-26)** — Deploy @239 ✅. Implementado: (1) `exportacao_engine.gs` — `gerarSNIIC()` REESCRITO com 6 seções reais: Identificação (nome/tipo/CNPJ/município/UF/capacidade de espaços), Funcionamento (dias/semana, horários), Recursos Humanos (efetivos+bolsistas+voluntários+agentes culturais), Atividades por categoria SNIIC (8 categorias com mapeamento de keywords: artes cênicas/visuais/audiovisual/música/formação/humanidades/patrimônio/outros) + distribuição mensal (12 meses), Público Atendido (inscrições/presenças/gratuito/pago + faixas etárias 0-12/13-17/18-29/30-59/60+ e PcD + NPS médio), Recursos Financeiros (total captado + desagregado federal/estadual/municipal/próprios/outros); `_gerarCsvSNIIC()` gera CSV com BOM UTF-8 formato Seção/Campo/Valor (6 seções, 40+ linhas) pronto para importação no portal MinC; (2) `index.html` — card SNIIC adicionado na aba Exportações do Financeiro (junto com SALIC e PNAB): input de ano + botão "Gerar SNIIC" + `div#exp-sniic-fin-resultado`; `ExportacoesUI` expandido com `gerarSNIIC()`: captura ano, `BtnGuard.wrap`, chama `GAS.exportacao.sniic`, inicia download automático do CSV, renderiza dashboard inline (`_renderSNIICResumo`: 6 stat cards + tabela de categorias + barras mensais); `aoAbrir()` inicializa `exp-sniic-fin-ano`; (3) `PublicoUI.exportarSNIIC()` REESCRITO: dashboard rico com 6 stat cards (Ações/Inscrições/Presenças/NPS/Equipe/Captado), barras de categoria com percentual visual, barras mensais, badges de faixas etárias coloridos, botão "CSV" para re-download manual (`_sniicCsvCache`); download automático ao gerar; `_downloadSNIICCsv` exposto na API pública. **Auditoria CLAUDE.md**: BtnGuard nos 2 botões SNIIC ✅ | GAS.exportacao.sniic binding existente ✅ | nenhum modal novo ✅ | nenhum prompt()/confirm() ✅ | FsmGuardian n/a (somente leitura) ✅. **Próximo**: smoke-test browser: (a) Público → Exportações → ano 2026 → Gerar SNIIC → dashboard aparece + CSV baixa automaticamente; (b) Financeiro → Exportações → card SNIIC → Gerar → resultado aparece + CSV baixa; (c) F12 → zero erros | BtnGuard.auditar() → "✅ todos protegidos".
